...
function generateExternalAIPrompt({ stepNum, stepName, authorProfile, workProfile, currentOutput }) {
  ...
  const firstResponseInstructions = isLongOutputStep
    ? `※ このSTEPは出力が大量（複数章/節）です。**全章を一度に改善しようとせず、深く1箇所に絞る方針**で動いてください。

1. 「現在の出力」「著者プロファイル」「書籍プロファイル」を**全文読み込む**
2. あなた自身の専門家視点で、**最も改善余地のある章（または節）を1つだけ特定**する
   - 例：核メッセージとの整合性が弱い章／読者の痛みとの接続が薄い節／一般論で具体性がない章 など
3. その章/節だけを深く改善した3案を提示する
   - 各案の冒頭に「【対象】 第N章「xxx」」と明記する
   - 各案には**その章/節の改訂版完全形だけ**を含める（他の章は変更しない、出力にも含めない）
   - 各案には「コンセプト名」「狙い」「トレードオフ」を1〜2行で添える
4. 最後にユーザーへ「気に入った案の改訂版を、出力欄の該当章 \`=== 第N章 xxx ===\` の位置と置き換えてください」と案内する
5. ユーザーが「他の章も見たい」「○○章も改善して」と言ったら、その章を対象に同じ方針で3案
6. ユーザーが「もっと深く」「別観点で」と言ったら、同じ章で違う方向性の3案`
    : `1. 「現在の出力」「著者プロファイル」「書籍プロファイル」を**全文読み込む**
        ...
        <div style={{ padding: 14 }}>
          ...
          {(stepNum === 7 || stepNum === 8) && (
            <div style={{ fontSize: 12.5, color: C.gold, lineHeight: 1.7, marginBottom: 10, padding: "8px 12px", background: C.goldPale, border: `1px solid ${C.goldLight}`, borderRadius: 4 }}>
              ...
              <strong>「最も改善余地のある1章/1節だけを深く改善する3案」</strong>を返してもらう設計になっています。
              気に入った案の改訂版を、出力欄の該当章（<code>=== 第N章 xxx ===</code> の位置）と置き換えてください。
            </div>
          )}
