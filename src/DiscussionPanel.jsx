...
4. **STEPごとの具体的アクションリスト** ← 最重要

   レビューを抽象論で終わらせないでください。**ユーザーが明日何をすればいいか**まで分解します。
   このツールには以下の9つのSTEPがあります：

   - STEP0: 著者プロファイル
   - STEP1: 書籍プロファイル草案
   - STEP2: 市場検証→書籍プロファイル確定
   - STEP3: エピソードインタビュー要約
   - STEP4: タイトル・サブタイトル作成
   - STEP5: 目次作成
   - STEP6: 章構成作成
   ...
   修正が必要なSTEPごとに、以下のフォーマットで書いてください：

   ─────────
   【優先度: 高】対象STEP: STEP6 章構成作成
   何を直すか: 第5章の「AI出版プロデューサー」言及を削除し、代わりに具体的なAIプロンプト例を3〜5個追加
   なぜ直すか: 読者が「自分でもできる」と確信する前にサービス勧誘が入ると冷める
   期待される効果: 誘導臭の除去 + 第5章の実用性向上
   ★ コピペ用改善指示文（このSTEPの再生成時に使える1段落）:
     「第5章は「AIを伴走者として使う章」として再設計してください。サービス名（AI出版プロデューサー、Life Book Navigator）への言及は完全に削除し、代わりに（1）詰まりを崩すAIプロンプト例3つ、（2）AIが問い返してくる対話例1つ、（3）AIに丸投げせず判断は自分でする使い方ルール、を入れて、読者が「自分でもAIを伴走者にできる」と感じる構成にしてください。」
   ─────────

   このフォーマットで、修正が必要な全STEPについて書いてください（最低3個、最大7個まで）。
   優先度は「高 / 中 / 低」のいずれか。
   コピペ用改善指示文は、ツールの「改善要望」入力欄や、外部AIへの追加指示としてそのまま使える1段落にしてください。

5. **総評**
   建設的なまとめ。「このまま進めるべきか」「一度立ち止まるべきか」の判断。
   どのSTEPから着手するのが効率的かも一言。


率直に。建前抜きで。ただし建設的に。
**「アクションリスト」と「コピペ用改善指示文」を必ず含めること**。これが無いとユーザーが何をすればいいか分からない。
`;
}
        ...
        <div style={{ padding: 14 }}>
          ...
          {mode === "review" && (
            <div style={{ fontSize: 12.5, color: C.textSub, lineHeight: 1.7, marginBottom: 10, padding: "8px 12px", background: "#fff7e6", border: "1px solid #ffd591", borderRadius: 4 }}>
              ...
              読者像／コンセプト／商業誘導性／差別化／橋渡し設計／致命的欠陥 を評価し、最後に
              <strong>「STEPごとの具体的アクション + コピペ用改善指示文」</strong>を返してくれます。
              <br />
              <strong style={{ color: C.gold }}>使い方</strong>：レビュー結果に出てくる「★ コピペ用改善指示文」を、該当STEPに移動してそのSTEPの相談機能（💡改善案）に貼り付けるか、出力データを手動で編集する材料として使ってください。
            </div>
          )}
