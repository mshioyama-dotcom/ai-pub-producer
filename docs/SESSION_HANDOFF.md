...
> **このセクションは「複数セッションが同じファイルを同時に書き換える事故を防ぐ」ための共有ホワイトボードです。**
> 大規模な改修を始める前に、必ず以下に方針・期間・影響ファイルを追記してから着手してください。
> 完了したら該当エントリを削除します。
> 新セッション開始時にここを確認し、同じ領域を触らないようにしてください。

### 📋 現在進行中の改修

- **なし**（2026-05-18 時点・MVP 完成）

### 📌 2026-05-18 時点の重要な実装履歴

**STEP11/12/13 を MVP として実装完了**:
- **STEP11**（X投稿生成 / SNS用短文記事）：1投稿/回イテレーション設計、毎日投稿型配分、gpt-5.2、Twitter prior回避のため「短文記事」リフレーミング
- **STEP12**（本の改善提案）：旧設計 STEP14 を改名＋位置づけ絞り込み、Real-Time Amazon Data API（RapidAPI）使用、Keepa 不採用、3セクション出力
- **STEP13**（出版経験の振り返り）：旧設計 STEP15 を改名＋位置づけ変更、著者プロファイル更新版を主役、次回作テーマは参考情報のみ、自動転記しない（著者の判断尊重）
...
```
- **2026-05-XX〜2026-05-XX: STEP2 を Amazon API キーワード絞り込みに再設計（v4改修）**
  - 影響ファイル: src/App.jsx (Step2Page), api/amazon-* (新規), dify/STEP2_*.yml
  - 担当セッション: ユーザー本人 (2026-05-15)
  - 完了済み（commit a8c9337, 1179f14 等）
```

### ⚠️ 過去に起きた事故（教訓）

- **2026-05-15〜16**: 別セッションで STEP2 を「Keepa 廃止して整合性チェック型」に再設計していた最中に、別セッションで「Amazon API キーワード絞り込み型」（v4改修）に再設計するコミットが積まれ、前者の改修が事実上無効化される事故が発生。再発防止のため、`.claude/settings.json` に `git push` 前 fetch 自動チェック hook を導入。

---

## 🚨 新セクションで最初に必ず確認すること（最重要）
