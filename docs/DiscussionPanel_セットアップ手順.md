...
| **STEP4** | ✅ **完全動作** | 【6】改善要望機構を実装済み |
| **STEP5** | ✅ **完全動作** | 【6】改善要望機構を実装済み |
| STEP6 | ⚠️ 部分動作 | YML変数のみ追加。プロンプトの【6】未実装（iterator型のため独自対応必要） |
| STEP7 | ⚠️ 部分動作 | 同上。YML変数のみ追加 |
| STEP8 | ⚠️ 部分動作 | 同上。本文作成のため出力フォーマットも独特 |
| STEP9 | ⚠️ 部分動作 | 同上 |

部分動作（⚠️）のSTEPでも、Claude Sonnet 4.6 は文脈から推測してそれなりに反応する可能性が高いですが、STEP4・STEP5 のような厳密な動作は保証されません。
...
- STEP6〜9 の各プロンプトに改善要望機構を追加（各STEPの構造に合わせた個別実装が必要）
