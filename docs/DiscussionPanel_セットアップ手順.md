...
## Phase 2：✨ この方針で再生成（実装済み）

DiscussionPanel に **「✨ この方針で再生成」** ボタンが追加されました。これは相談ログをAIに要約させ、その方針を `improvement_request` として STEP本体の再生成APIに渡す機能です。

### ユーザーから見たフロー

1. STEP4などで出力を生成 → 違和感を覚える
2. 「💬 この出力について相談する」を開いてAIと議論
3. 数往復で改善方針を固める
4. **「✨ この方針で再生成」** ボタンをクリック
5. AIが議論ログを「改善要望」として1段落に自動要約
6. 確認ダイアログで内容を確認 → OK
7. STEP本体の再生成APIが呼ばれる（YMLの【6】改善要望機構が動作）
8. 出力データが**修正版に自動更新される**（フォーマット・ルールは元のSTEP YMLが保証）

### 動作するSTEP一覧

| STEP | Phase 2 動作 | 備考 |
|---|---|---|
| STEP0 | ❌ なし | 専用ページ・DiscussionPanel未搭載 |
| STEP1 | ❌ なし | チャット型（メインチャットで議論を続ければよい） |
| STEP2 | ❌ なし | 入力（Amazon HTML）が複雑なため対象外 |
| **STEP4 タイトル** | ✅ **完全動作** | 【6】改善要望機構を実装済み |
| **STEP5 目次** | ✅ **完全動作** | 【6】改善要望機構を実装済み |
| **STEP6 章構成** | ✅ **完全動作** | iterator型対応の改善要望機構を実装。章単位で「コピー」「修正」を判定 |
| **STEP7 詳細プロット** | ✅ **完全動作** | 階層構造（章→節→項）対応の改善要望機構を実装 |
| **STEP8 本文作成** | ✅ **完全動作** | 散文書き直し型の改善要望機構を実装。loop型のため再生成も全節をループ処理 |
| **STEP9 Amazon説明文** | ✅ **完全動作** | 段落単位の改善要望機構を実装 |

各STEPの出力構造に合わせて改善要望機構をカスタマイズしているため、修正の精度が高く維持されます。

### YML 再import が必要

各STEP YMLに新しい入力変数（`previous_output`, `improvement_request`）を追加したため、Dify Cloudへの**再import → 公開**が必要です。

**作業対象（必須・全6本）：**
- [ ] STEP4_タイトル・サブタイトル作成_C案 → 再import → 公開
- [ ] STEP5_目次作成_C案 → 再import → 公開
- [ ] STEP6_章構成作成_C案 → 再import → 公開
- [ ] STEP7_詳細プロット作成_C案 → 再import → 公開
- [ ] STEP8_本文作成_C案 → 再import → 公開
- [ ] STEP9_Amazon説明文作成_C案 → 再import → 公開

### Phase 3 候補（次のステップ）

- サーバー側でのユーザー単位の使用量制限（認証 + DB 導入後・サブスク本格化時）
- 議論履歴の閲覧UI（過去の議論を見返す）
- Tier別の動的なターン数上限（Lite=5, Standard=10, Pro=20）
- 利用ログから原価可視化ダッシュボード
